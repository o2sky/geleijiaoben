//2026/06/29 (converted for Surge)

/*
@Name：PingMe 自动化签到+视频奖励（Surge 版）
@Author：怎么肥事（原作者，QX 脚本）/ 转换：Surge 适配

======================== Surge 配置（写入 Surge 主配置文件） ========================

[Script]
PingMe抓包 = type=http-request,pattern=^https:\/\/api\.pingmeapp\.net\/app\/queryBalanceAndBonus,script-path=https://raw.githubusercontent.com/ZenmoFeiShi/Qx/main/PingMe.surge.js,requires-body=false,timeout=60,tag=PingMe抓包
PingMe签到 = type=cron,cronexp="30 8,20 * * *",script-path=https://raw.githubusercontent.com/ZenmoFeiShi/Qx/main/PingMe.surge.js,timeout=120,tag=PingMe签到
...
使用说明：
1. 把上面 [Script] 和 [MITM] 两段内容分别合并进你 Surge 的对应分组（不要整段覆盖，用 %APPEND% 追加 hostname）。
2. script-path 建议改成你自己 fork/托管后的地址（把本文件传到你的 GitHub 仓库，用 raw 链接）。
3. 先打开 PingMe App 触发一次请求完成"抓包入库"，再手动运行一次 PingMe签到 脚本验证签到逻辑。
4. Surge 需要在 Boss/Panel 或脚本里点一次“运行”触发 cron 脚本的首次授权弹窗（部分版本）。

主要改动点（相对原 QX 脚本）：
- $prefs.valueForKey / setValueForKey  →  $persistentStore.read / write
- $notify(title, subtitle, body)       →  $notification.post(title, subtitle, body)
- $task.fetch(...).then/.catch (Promise) → 用 $httpClient.get 包一层 Promise（Surge 原生是回调式 API，没有 $task）
- 其余业务逻辑（MD5 签名、UA 随机化、签到/查询/视频奖励流程、账号存储结构）保持不变
*/
...
// ---- 存储：QX 的 $prefs → Surge 的 $persistentStore ----
function loadStore() {
...
// ---- 通知：QX 的 $notify → Surge 的 $notification.post ----
...
// ---- 网络请求：QX 的 $task.fetch（Promise）→ 用 $httpClient.get 包一层 Promise ----
...
// ================= 入口：http-request（抓包）分支 与 cron（签到）分支，判断方式和 QX 一致 =================
if (typeof $request !== 'undefined' && $request) {
  // 抓包分支：由 [Script] 里的 http-request 脚本触发
